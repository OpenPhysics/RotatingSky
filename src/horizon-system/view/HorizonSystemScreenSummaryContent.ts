/**
 * HorizonSystemScreenSummaryContent.ts
 *
 ...
 * localized pattern with the observer's latitude, the number of stars, and the
 * local sidereal time, and updates automatically as the model changes.
 */
...
export class HorizonSystemScreenSummaryContent extends ScreenSummaryContent {
  public constructor(model: HorizonSystemModel) {
    const a11y = StringManager.getInstance().getHorizonSystemA11yStrings();
    const sky = model.sky;
    ...
    const currentDetails = new PatternStringProperty(
      ...
      { decimalPlaces: { latitude: 0, starCount: 0, time: 1 } },
    );
