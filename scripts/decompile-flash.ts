 ...
 *
 * Run with:
 *   npm run decompile                 # the Rotating Sky Explorer → NAAP/decompiled/
 *   npm run decompile -- --all        # all Rotating-Sky-relevant movies
 ...
 * These cover the NAAP "Rotating Sky" (motion2) lab and its building blocks.
 */
const SKY_TARGETS = [
  // The main simulator (ce_hc.swf in the published lab) — primary source of truth.
  "celHorComp/celHorComp039-D.swf",
  // The two view components the explorer composes: flat horizon dome + celestial sphere.
  "flatSkyView/flatSkyView004.swf",
  "simpleFlatSkyMap/simpleFlatSkyMap007.swf",
  "skyMap/skyMap028.swf",
  "celestial_sphere/celestial_sphere140.swf",
  "rotatingSphere/rotatingSphere001.swf",
  // Background demos that ship with the lab.
  "declination_ranges/declination_ranges002.swf",
  "siderealTimeAndHourAngleDemo/siderealTimeAndHourAngleDemo004-B.swf",
  "eduSkyMotion/eduSkyMotion002.swf",
  "celestialHorizon/celestialHorizon004.swf",
  "diurnalMotion/diurnalMotion004.swf",
  // Zodiac / ecliptic context (has readable .as already, but include for completeness).
  "zodiacSimulator/zodiacSimulator006.swf",
].map((p) => join(flashRoot, p));

/** The main explorer decompiled by default when no targets are given. */
const DEFAULT_TARGETS = SKY_TARGETS.slice(0, 1);
...
Usage:
  npm run decompile                  Decompile the Rotating Sky Explorer → NAAP/decompiled/
  npm run decompile -- --all         Decompile all Rotating-Sky-relevant movies
  npm run decompile -- <path>…       Decompile specific .swf / .fla / folder(s)

Flags:
  --all              Decompile the curated set of Rotating-Sky SWFs
