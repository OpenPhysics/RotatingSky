...
main.ts
  └─ RotatingSkyScreen            (Screen<RotatingSkyModel, RotatingSkyScreenView>)
       ├─ RotatingSkyModel         owns all state
       └─ RotatingSkyScreenView    owns all visuals
```
...
Mirror the structure of `src/rotating-sky-screen/`:
...
const icon = new ScreenIcon(
  new Rectangle(0, 0, 548, 373, { fill: RotatingSkyColors.accentColorProperty }),
